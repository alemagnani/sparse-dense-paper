...
\caption{Recall@10 vs.\ per-query latency (log-x), single-threaded with warm-up; HNSW configured with the standard FAISS recipe $M{=}16$, $\mathrm{efC}{=}200$, sweeping $\mathrm{efSearch}\in\{50,\dots,16000\}$ (blue curve, annotated). Flat is one red square. Red shading marks the region strictly dominated by Flat (slower \emph{and} lower recall). Flat upper-bounds recall by construction. On standard subsets HNSW at $\mathrm{efS}{=}50$--$100$ is $2$--$5\times$ faster than Flat at $\ge 92\%$ of Flat's recall---the canonical ANN sweet spot. On ID subsets the HNSW curve climbs slowly: HD-ID requires $\mathrm{efS}{=}8000$ ($40\times$ Flat's latency) to reach $95\%$ of Flat's recall; MM-ID needs $\mathrm{efS}{=}16000$ ($81\times$).}
\label{fig:pareto}
\end{figure*}

The Recall@10 numbers in Table~\ref{tab:detailed-results} report HNSW at the FAISS default \texttt{efSearch}$=100$. The complete picture requires sweeping \texttt{efSearch} and measuring the latency cost of approaching Flat. Fig.~\ref{fig:pareto} does this for each (dataset, subset) with the standard HNSW recipe $M{=}16$, $\mathrm{efC}{=}200$. The HNSW curve has a qualitatively different shape on standard vs.\ spear-fishing subsets.

\paragraph{Cost-to-close: the corpus-invariant signal.}
We summarize the figure by the \texttt{efSearch} value required to reach $95\%$ of Flat's Recall@10, with the resulting latency relative to Flat:
\begin{center}\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrr}
\toprule
Subset & efS for $95\%$ Flat & ms/q & vs.\ Flat \\
\midrule
HD-standard  & $100$    & $0.39$  & $2.7\times$ \emph{faster} \\
HD-rare      & $2{,}000$ & $6.94$  & $6.5\times$ slower \\
HD-ID        & $\mathbf{8{,}000}$ & $\mathbf{41.9}$ & $\mathbf{40\times}$ slower \\
MM-standard  & $1{,}000$ & $3.48$  & $2.1\times$ slower \\
MM-rare      & $16{,}000$ & $129.6$ & $79\times$ slower \\
MM-ID        & $\mathbf{16{,}000}$ & $\mathbf{132.0}$ & $\mathbf{81\times}$ slower \\
\bottomrule
\end{tabular}
\end{center}
Closing the ID gap to $95\%$ of Flat recall requires $80$--$160\times$ more \texttt{efSearch} budget than the standard subset on the same data---an $40$--$80\times$ multiplicative latency factor at our corpus sizes. The corpus-invariant signal is the \texttt{efSearch} ratio itself: ANN search cost grows roughly linearly with \texttt{efSearch}, so this multiplier carries through any scale of deployment.

\paragraph{Standard queries: the canonical ANN sweet spot.}
On HD-standard HNSW efS$=100$ delivers $96\%$ of Flat's recall at $0.39$ ms/q---$2.7\times$ faster than Flat's $1.07$ ms/q. On MM-standard the same operating point delivers $82\%$ recall at $0.39$ ms/q ($4.2\times$ faster). At efS$=50$, HD-standard achieves $4.7\times$ speedup at $92\%$ recall. This is exactly what HNSW is designed for and used in production.

\paragraph{Corpus-size caveat.}
At our $N{\approx}10^5$, FAISS Flat is fast ($\sim 1$--$1.6$ ms/q) because dense matrix--vector products parallelize well across SIMD; HNSW's logarithmic-speedup advantage is modest. At $N{=}10^7$ Flat would slow linearly (${\sim}100$ ms/q) while HNSW efS$=100$ grows logarithmically, so HNSW would clearly beat Flat in latency on \emph{standard} queries. The \emph{cost-to-close ratio} is what generalizes: at any scale, reaching Flat-level recall on ID requires $80$--$160\times$ the \texttt{efSearch} budget of standard, and therefore consumes a substantial fraction of HNSW's speedup advantage.
