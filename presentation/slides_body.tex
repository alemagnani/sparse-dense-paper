...
\begin{frame}{The bound: when does the centroid signal survive?}
  ...
  \textbf{Step 2 --- Prevalence.} Let $n_{t^*,C}$ = \# docs \emph{in $C$}
  containing $t^*$. Only these contribute to the $t^*$-direction of $c_C$:
  \[
    c_C \big|_{t^*}
    \;\approx\;
    \underbrace{\tfrac{n_{t^*, C}}{|C|}}_{\displaystyle p\;=\;\text{prevalence of }t^*\text{ in }C}
    \;\cdot\; \alpha_{t^*}.
  \]
  ...
  \begin{block}{Survival condition}
    ...
    Relevant centroid picked iff $\;\alpha_{t^*}\cdot p \gg \epsilon\;$
