...
\begin{frame}{Experimental setup}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item HNSW ($M{=}16$, $\texttt{efC}{=}200$, \texttt{efS} sweep)
      \item BM25 (Pyserini)
    \end{itemize}
  \end{columns}
...
\begin{frame}{Headline result: the gap}
  ...
  \begin{itemize}
    \item Flat$\to$HNSW gap (relative):\;
          HD \{standard \textbf{1.8\%}, rare \textbf{17.5\%}, \alert{id \textbf{62.6\%}}\};\;
          MM \{standard \textbf{5.3\%}, rare \textbf{19.0\%}, \alert{id \textbf{59.2\%}}\}.
    \item Paired-bootstrap $95\%$ CI on Flat$-$HNSW excludes zero everywhere
          (HD-ID $+0.576$, MM-ID $+0.138$).
    \item \emph{MS MARCO caveat:} BM25 $>$ Flat dense on standard/rare reflects a
          representation limit of unsupervised trigrams---our claim is the
          Flat$\to$ANN \emph{gap}, well-defined at any absolute level.
  \end{itemize}
\end{frame}
...
\begin{frame}{Indirect evidence: IVF cluster granularity}
  \begin{columns}[T]
    ...
    \medskip
    Asymmetric improvement is exactly what the centroid-dilution bound (Eq.~1) predicts.
  \end{columns}
\end{frame}
...
\begin{frame}{Direct evidence: measure the centroid signal}
  ...
  \begin{columns}[T]
    ...
    centroid signal disappears into the JL noise floor exactly as the bound (Eq.~1) predicts.\\[0.3em]
...
\begin{frame}{The empty-result rate}
  ...
  \begin{itemize}
    \item HD-id Flat \textbf{13\%} $\to$ HNSW \alert{\textbf{71\%}} return nothing relevant in top-10.
    \item MM-id Flat \textbf{78\%} $\to$ HNSW \alert{\textbf{92\%}}.
    \item Standard subsets: $\le 2$ percentage points either way on both datasets.
    \item \emph{This is the production-visible symptom.}
  \end{itemize}
\end{frame}

%% =====================================================================
\begin{frame}{The recall--latency trade-off changes shape}
  \centering
  \includegraphics[width=0.88\textwidth]{figures/pareto_recall_vs_latency.png}

  \vspace{-0.6em}
  {\scriptsize HNSW efSearch sweep (standard FAISS recipe $M{=}16$, $\textrm{efC}{=}200$) vs.\ Flat (red square). Red region = strictly Pareto-dominated by Flat.}
\end{frame}

%% =====================================================================
\begin{frame}{The cost-to-close ratio (corpus-invariant)}
  \texttt{efSearch} needed to reach $95\%$ of Flat's Recall@10:
  \vspace{0.3em}
  \begin{center}
    \small
    \setlength{\tabcolsep}{6pt}
    \begin{tabular}{lrrr}
      ...
      \midrule
      HD-standard & $100$    & $0.39$  & {\color{good}\textbf{2.7$\times$ faster}} \\
      HD-rare     & $2{,}000$ & $6.94$  & $6.5\times$ slower \\
      HD-{\color{accent}id}  & {\color{accent}\textbf{$8{,}000$}} & {\color{accent}$41.9$} & {\color{accent}\textbf{$40\times$ slower}} \\
      \midrule
      MM-standard & $1{,}000$ & $3.48$  & $2.1\times$ slower \\
      MM-rare     & $16{,}000$ & $129.6$ & $79\times$ slower \\
      MM-{\color{accent}id}  & {\color{accent}\textbf{$16{,}000$}} & {\color{accent}$132.0$} & {\color{accent}\textbf{$81\times$ slower}} \\
      \bottomrule
    \end{tabular}
  \end{center}
  ...
  \vspace{0.6em}
  \begin{itemize}
    \item Standard queries: HNSW gives the canonical ANN speedup.
    \item ID queries: need \alert{$80$--$160\times$ more efSearch budget} than standard.
    \item Search cost scales $\sim$linearly in efSearch $\Rightarrow$ this ratio
          \emph{generalizes to any corpus size}.
  \end{itemize}
\end{frame}
...
\begin{frame}{What this means for production}
  \begin{block}{Two ways to read the gap}
    \begin{itemize}
      \item \textbf{Default efSearch:} HNSW forfeits $\sim 63\%$ of achievable
            recall on identifier queries, $\sim 18\%$ on rare, $\sim 2\%$ on standard.
            \emph{Visible:} 71\% of HD-ID HNSW queries return nothing relevant.
      \item \textbf{Match Flat recall:} on ID you must pay $40$--$80\times$ Flat
            latency. ANN's speedup advantage \emph{evaporates} on the population
            that needs reliability the most.
    \end{itemize}
  \end{block}
  ...
  \begin{enumerate}
    ...
    \item \emph{Hybrid retrieval}: sparse for precision on rare tokens, dense for
          semantic generalization---now with a quantitative justification.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Takeaways}
  \begin{enumerate}
    ...
    \item ANN indices introduce a \emph{second}, structurally different failure
          mode concentrated on rare-token / identifier queries.
    \item We formalized this as \textbf{indexing bias} + \textbf{centroid-dilution
          bound}, then directly measured the centroid signal per query
          ($\sim 40\times$ collapse from standard to ID).
    \item Reaching Flat-equivalent recall on ID requires $80$--$160\times$ more
          \texttt{efSearch} budget than on standard---the corpus-invariant
          quantification of the ANN value-proposition collapse.
