...
\begin{frame}{The idealized case: BM25 \emph{is} embedding search}
  \textbf{Reframe BM25 as a dense inner product} in a huge, discrete
  embedding space $\mathbb{R}^V$ (one dimension per token, $V \approx 10^5$).

  \medskip
  \begin{itemize}
    \item Each doc $d$ is a sparse vector $f(d) \in \mathbb{R}^V$ with
          $f(d)_t = \alpha_t$ (IDF weight) if $t \in d$, else $0$.
    \item Query $q$ dominated by rare token $t^*$: $q$ has weight
          $\alpha_{t^*}$ in that direction.
    \item Inner product with a relevant doc $d_{\text{rel}}$ (contains $t^*$):
          \[
            \langle q, f(d_{\text{rel}})\rangle \;=\; \alpha_{t^*}^{\,2}
            \qquad \text{(big --- } \alpha_{t^*} = \log\!\big(M/n_{t^*}\big)\text{).}
          \]
    \item Inner product with an irrelevant doc: essentially $0$.
    \item \alert{Perfect separation on spear-fishing queries.} This is
          exactly why BM25 works. It is a nearest-neighbor search in
          $\mathbb{R}^V$.
  \end{itemize}

  \medskip
  \textbf{The catch}: $V$ is huge. Storing and searching $\mathbb{R}^V$ is
  what BM25 does with an inverted list --- practical, but the wrong shape for
  hybrid or pure dense. To use ANN we must compress $\mathbb{R}^V \to
  \mathbb{R}^d$ with $d \ll V$.

  \emph{The rest of this talk is what compression costs.}
\end{frame}

%% =====================================================================
\begin{frame}{Compressing $\mathbb{R}^V \to \mathbb{R}^d$: Johnson--Lindenstrauss}
  Random linear map $A: \mathbb{R}^V \to \mathbb{R}^d$, $d \in \{256, 512, 1024\}$.
  ...
  \begin{itemize}
    \item Any inner product with magnitude \alert{$\gg \epsilon$} is preserved.
          $\Rightarrow$ Spear-fishing signal $\alpha_{t^*}^{\,2}$ from the previous
          slide easily clears $\epsilon$: \textbf{Flat search on the projected
          embedding is a clean oracle.}
    \item Any inner product with magnitude \alert{$\lesssim \epsilon$}
          is drowned in projection noise --- indistinguishable from random.
    \item Bigger $d$ shrinks $\epsilon$; nothing else does.
  \end{itemize}

  \medskip
  \small So after JL alone, spear-fishing still works. The failure emerges when
  we add the \emph{second} compression --- clustering --- on top. That is IVF.
\end{frame}
...
\begin{frame}{The bound: when does the centroid signal survive?}
  \footnotesize\emph{Recall}: in $\mathbb{R}^V$, $\langle q, f(d_{\text{rel}})\rangle = \alpha_{t^*}^{\,2}$; JL preserves it. IVF selects clusters by \textbf{centroid} inner product $\Rightarrow$ average kicks in.
  ...
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item $M$ = corpus size (\# docs)
      \item $N$ = \# IVF clusters
      \item $C$ = one cluster, $|C| \approx M/N$
      \item $t^*$ = rare token dominating the query
      \item $n_{t^*}$ = \# docs in corpus with $t^*$
      \item $n_{t^*,C}$ = \# docs \emph{in $C$} with $t^*$
      \item $\alpha_{t^*} = \log(M/n_{t^*})$ (IDF)
      \item $\epsilon$ = JL noise floor
    \end{itemize}

    \column{0.58\textwidth}
    \small
    \textbf{Step 1.} Centroid = average of docs in the cluster:
    \[
      c_C \;=\; \tfrac{1}{|C|}\sum_{d\in C} f(d).
    \]

    \textbf{Step 2.} $t^*$ contributes to $c_C$ only through the $n_{t^*,C}$
    docs in $C$ that contain it:
    \[
      c_C \big|_{t^*}
      \;\approx\; \underbrace{\tfrac{n_{t^*,C}}{|C|}}_{p \;=\; \text{prevalence}} \cdot\, \alpha_{t^*}.
    \]
    Rare token $\Rightarrow p$ tiny.

    \textbf{Step 3.} Query dominated by $t^*$ (weight $\alpha_{t^*}$):
    \[
      \boxed{\;\langle c_C,\, q\rangle \;\approx\; \alpha_{t^*}^{\,2}\cdot p\;}
    \]
  \end{columns}

  \begin{block}{Survival condition}
    \footnotesize\centering
    Relevant centroid recognizable iff $\;\alpha_{t^*}^{\,2}\cdot p \gg \epsilon.\;$
    Rare: $\alpha$ big, $p$ tiny $\Rightarrow$ product $<\epsilon$ $\Rightarrow$
    wrong cluster picked.
    \emph{Bigger $d$ helps $\epsilon$; nothing helps $p$.}
  \end{block}
\end{frame}
...
\begin{frame}{Direct evidence: measure the centroid signal}
  ...
  \textbf{Tie to the bound.} $\langle c,q\rangle \approx \alpha_{t^*}^2 p$.
